...
export async function createContact(
  ...
  try {
    ...
    await prisma.contact.create({ data: { ...data, workspaceId: user.workspaceId } });
  } catch (error) {
...
export async function updateContact(
  ...
  try {
    const user = await getCurrentUser();
    if (!user) throw new Error("Unauthorized");
    await prisma.contact.update({ where: { id, workspaceId: user.workspaceId }, data });
  } catch (error) {
    if (isUniqueEmailError(error)) {
      return { errors: { email: "A contact with this email already exists." }, values };
    }
...
export async function deleteContact(formData: FormData) {
  ...
  const user = await getCurrentUser();
  if (!user) throw new Error("Unauthorized");
  await prisma.contact.delete({ where: { id, workspaceId: user.workspaceId } });
  revalidatePath("/contacts");
}
