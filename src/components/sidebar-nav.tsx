...
const NAV: NavItem[] = [
  ["Dashboard","/dashboard","grid"], ["Prospecting","/prospecting","compass"],
...
export function SidebarNav({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <nav className="flex flex-col gap-0.5 overflow-y-auto pr-1">
    {NAV.map(item => {
      ...
      return <Link key={item.href} href={item.href} onClick={onNavigate}
        ...
        <span className={active ? "text-accent" : "text-zinc-400 group-hover:text-zinc-600"}>{item.icon}</span>
